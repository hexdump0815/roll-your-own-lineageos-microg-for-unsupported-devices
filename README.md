...
* n8010 - samsung galaxy note 10.1 tablet
...
while experimenting around with new manifest files i sometimes came across errors like "Can not locate config makefile for product" although the manifest looked like it has all required in and should work. in the end it was required to start a build from scratch (i.e. removing the lineage/CM_14_1 dir beforehand) - looks like the repo sync does not handle changes in the manifest file well enough and a fresh start seems to be a good idea.
